...
class Cache {
  constructor(eventEmitter, cacheSize, timeToLive, redisConfig = {}) {
    ...
    let redis = redisConfig.active || false;
    if (timeToLive === 0) {
      console.warn('dm-cache deactivating redis: ttl 0 with redis not supported');
      redis = false;
    }
    ...
    this[entryCacheSymbol] = cacheManager.multiCaching(
      [this[entryCacheMemorySymbol], redis ? cacheManager.caching(cacheOptionsRedis) : undefined]
      .filter(x => !!x));
    this[modelCacheSymbol] = cacheManager.multiCaching(
      [this[modelCacheMemorySymbol], redis ? cacheManager.caching(cacheOptionsRedis) : undefined]
      .filter(x => !!x));
