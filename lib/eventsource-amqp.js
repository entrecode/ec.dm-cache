...
class EventSourceAMQP {
  ...
  set rabbitMQChannel(channelWrapper) {
    ...
    this[channelWrapperSymbol].addSetup((channel) => {
      channel
  ...
  [emitEventSymbol](message) {
    ...
    if (this[channelSymbol]) {
      this[channelSymbol].ack(message).catch((error) => {
        if (error.message.indexOf('Channel ended') !== -1) {
  ...
  watch(modelTitle, entryID) {
    ...
    if (entryID) {
      // watch entry
      if (
        !this[watchedModelsSymbol].has(modelTitle) && // if model is already watched, do nothing
        ...
        const entryWatcher = (channel) =>
          channel.bindQueue(this[queueSymbol], 'publicAPI', `${this.dataManagerShortID}.${modelTitle}.${entryID}.#`);
        this[channelWrapperSymbol].addSetup(entryWatcher);
      ...
      // watch model
      if (!this[watchedModelsSymbol].has(modelTitle)) {
        // if model is already watched, do nothing
        const modelWatcher = (channel) =>
          channel.bindQueue(this[queueSymbol], 'publicAPI', `${this.dataManagerShortID}.${modelTitle}.#`);
        this[channelWrapperSymbol].addSetup(modelWatcher);
        ...
        if (this[watchedEntriesSymbol].has(modelTitle)) {
          ...
          this[watchedEntriesSymbol].get(modelTitle).forEach((entryWatcher, entryID) => {
            // TODO entries/values
            this[channelWrapperSymbol].removeSetup(entryWatcher);
            // TODO check if connected?
            this[channelSymbol].unbindQueue(
              this[queueSymbol],
              'publicAPI',
              `${this.dataManagerShortID}.${modelTitle}.${entryID}.#`,
            );
          });
