    ...
    if (entryID) {
      ...
      if (
        ...
        const entryWatcher = (channel) =>
          Promise.all([
            channel.assertQueue(this[queueNameSymbol], { exclusive: true }),
            channel.bindQueue(
              this[queueNameSymbol],
              'publicAPI',
      ...
      const modelWatcher = (channel) =>
        Promise.all([
          channel.assertQueue(this[queueNameSymbol], { exclusive: true }),
          channel.bindQueue(this[queueNameSymbol], 'publicAPI', `${this.dataManagerShortID}.${modelTitle}.#`),
        ]).catch((error) => {
