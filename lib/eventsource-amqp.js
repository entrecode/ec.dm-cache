  ...
  watch(modelTitle, entryID) {
    ...
    if (entryID) {
      ...
      if (!this[watchedModelsSymbol].has(modelTitle)) {
        ...
        if (this[watchedEntriesSymbol].has(modelTitle)) {
          // if we have watched for single entries before,
          this[watchedEntriesSymbol].get(modelTitle).forEach((
            entryID, // unbind them because they are unnecessary now
          ) =>
