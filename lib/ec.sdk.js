...
class SDKWrapper {
  ...
  findLinkedEntries(entry, levels = 1) {
    ...
    out = out.filter((link) => /[A-Fa-f0-9]{8}:[a-zA-Z0-9_\\-]{1,256}\/(?!_?creator)[a-zA-Z0-9_\\-]{1,256}/.test(link));
    out = out.map((link) => {
    ...
    out = out.reduce((array, links) => array.concat(links), []);
    out = out.filter((linkObject) => 'name' in linkObject);
    out = out.map((linkObject) => [linkObject.name, linkObject.href.split('=')[1]]);
